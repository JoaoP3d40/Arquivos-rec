...
def excluir_contato():
    ...
    resposta = messagebox.askokcancel(title:"Excluir contato", message:"Tem certeza que deseja excluir o contato?")
    ...
    os.remove("dados.csv")
    os.reneme(src: "temp.csv", dst:"dados.csv")
    limpar_campos()
    ler_contatos()
    else:
...
button_gravar.place(x=10, y=230, width=145, height=60)

button_excluir.config(font=("Arial", 16))
button_excluir.place(x=165, y=230, width=145, height=60)
